...
bbop.golr.manager = function (golr_loc, golr_conf_obj){
     ...
     *
     * Getter/setter for the query fields--the fields that are
	...
	// Also note the all the keys that we see.
	var seen_params = {};
	loop(in_params,
	     function(ip){
		 var key = ip[0];
	...
	var qf_holder = {}; // TODO: a better internal model of qf.
	loop(in_params,
	     function(ip){
		 var key = ip[0];
		 var val = ip[1];
	...
	// Now apply the qf weirdness.
	anchor.query_field_set(qf_holder);
