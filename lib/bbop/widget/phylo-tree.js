...
function renderer(parent, config){
    ...
    this.node_style = {
        ...
        transition: ("top " + this.config.transition_time + " ease-in-out,"
                     + "left " + this.config.transition_time + " ease-in-out")
    };
...
renderer.prototype.width_changed = function(parent_width) {
    ...
    console.log(this.container.style.cssText);
};
...
graph_pnode.prototype.set_position = function(x, y) {
    ...
    if (this.is_leaf) {
        ...
        // (px, py) position
        this.node_elem.style.top = (y - (this.height / 2)) + "px";
    } else {
