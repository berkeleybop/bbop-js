...
renderer.prototype.show_node_menu = function(node, node_elem) {
    var self = this;
    var menu_elem = this.node_menu_elem;

    menu_elem.html("<h4>" + node.label + "</h4>");

    var buttons = [];
    if (! node.is_leaf()) {
        buttons.push({
            text:  self.tree_renderer.subtree_hidden(node.id)
                ? "Show subtree" : "Hide subtree",
            click: function() {
                menu_elem.dialog("close");
                self.toggle_subtree_shown(node.id);
            }
        });
    }
    buttons.push({
        text:  self.tree_renderer.only_subtree_shown(node.id)
    ...
    var dia = menu_elem.dialog(diargs);
    menu_elem.show();
};
