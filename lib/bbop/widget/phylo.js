...
function renderer(parent, family_id, golr_loc, golr_conf, config) {
    this.parent = parent;
    var default_config = {
        // these are in pixels
    ...
    $(document).ready(function() {
        //temp hack
        self.process_family(global_graph);
        return;

        function searchCallback(response) {
            self.process_family(JSON.parse(response.documents()[0].phylo_graph));
        }

        var go = new bbop.golr.manager.jquery(golr_loc, golr_conf);
        go.register('search', 's', searchCallback);
        // all bioentities in a family
        go.set_personality('bbop_bio');
        go.add_query_filter('document_category', 'bioentity', ['*']);
        go.set_query("panther_family_label:" + family_id);
        go.search();
    });
}

renderer.prototype.process_family = function(pgraph) {
    ...
    tree_renderer.iterate(function(id, label, meta, dom_elem, children) {
        if ("true" == meta.speciation_p) {
            dom_elem.style.borderRadius = "5px";
