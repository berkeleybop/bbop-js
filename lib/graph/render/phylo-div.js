...
var graph_pnode = function(context, label, px, py, internal){
...
bbop.render.phylo.renderer = function (element_id){
    ...
    this.leaf_style = {
        position: "absolute",
        background: "#f1f1ff",
        "margin-top": "-" + ((this.box_height / 2) + 5) + "px",
        padding: "5px",
	...
	this._render_frame_width = this.container.innerWidth();

	this._render_internal_width =
	    this._render_frame_width
	    - (1.0 * renderer_context.box_width)

	var x_scale = this._render_internal_width / layout.max_distance;
	var y_scale = renderer_context.box_height * 2.0; // fixed y-scale

        var top_margin = (this.box_height / 2) + 2;
	...
	for( var nidi = 0; nidi < layout.node_list.length; nidi++ ){
	    ...
	    if( ! this._graph.is_leaf_node(node_id) ){
                phynode = new graph_pnode(this, node_id, lpx, lpy, true)
	    }else{
                phynode = new graph_pnode(this, node_id, lpx, lpy, false)
	    }
	...
	for( var ei = 0; ei < layout.edge_list.length; ei++ ){
        ...
        this.container.css({height: this._render_frame_height + "px"});
    }
};
