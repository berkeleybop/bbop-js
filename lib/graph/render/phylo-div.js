...
function divrenderer(parent, config){
...
divrenderer.prototype.set_styles = function(css_string) {
    if( ! this._style_node ){
...
divrenderer.prototype.display = function () {
    if( ! this.is_sorted() ) this.sort_tree("ladderize_up");
    ...
    this._measure_box_height(node_class, leaf_class);
    ...
    this.tree_height = ( (this.tree.leaves().length * this.row_height)
                         - this.config.box_spacing
                         + (this.config.parent_padding * 2) );

    this.parent.style.height = this.tree_height + "px";
    this.container.style.left = ( (this.config.node_size / 2)
...
divrenderer.prototype.width_changed = function(parent_width) {
    var leaves = this.tree.leaves();
    
    var potential_widths = new Array(leaves.length);
    for( var li = 0; li < leaves.length; li++ ){
        var leaf_id = leaves[li].id;
        var phynode = this._phynodes[leaf_id];
        var leaf_width = this._leaf_widths[leaf_id];

        // assumes leaf widths don't change after they're first determined
        if( leaf_width === undefined ){
            leaf_width = phynode.node_elem.offsetWidth;
            this._leaf_widths[leaf_id] = leaf_width;
        }

        potential_widths[li] =
            // dividing px by 100 because px is in percentage units
            (parent_width - leaf_width) / (phynode.px / 100);
    }
    this.container.style.width = ( min(potential_widths)
                                   - (this.config.node_size / 2)
                                   - (this.config.parent_padding * 2) ) + "px";
};

// sort the tree according to the given sort ordering
// see available_sorts for available sort arguments
divrenderer.prototype.sort_tree = function(sort) {
    if(sort == this._current_sort) return;
...
divrenderer.prototype.is_sorted = function() {
    return ( this._current_sort != null );
};

divrenderer.prototype.leaves = function() {
...
divrenderer.prototype._measure_box_height = function(node_class, leaf_class) {
    var fake_phynode = new graph_pnode("foo", 0, 0,
                                       node_class, leaf_class,
                                       0,
                                       true);
    this.parent.appendChild(fake_phynode.node_elem);
    this.config.box_height = fake_phynode.node_elem.offsetHeight;
    this.parent.removeChild(fake_phynode.node_elem);
};
...
graph_pnode.prototype.set_parent = function(parent, conn_class) {
    ...
    this.conn_elem.style.cssText = this_style;
};
...
tree.prototype.iterate_postorder = function(fun) {
    var roots = this.roots()
    for (var i = 0; i < roots.length; i++) {
        roots[i].iterate_postorder(fun);
    }
};
...
tree.prototype._summarize = function() {
    ...
    var leaves = [];
    this.iterate_postorder(function(node) {
        if( node.is_leaf() ) leaves.push(node);
    });
    this._leaves = leaves;
    
    this._max_distance =  max(this.traverse(
    ...
    return Array.prototype.concat.apply([], this.traverse(
        ...
        function(node, child_results, down_data) {
            ...
            for (var i = 0; i < child_results.length; i++) {
                ...
                // are for that child's descendants.
...
node.prototype.iterate_postorder = function(fun) {
    for (var i = 0; i < this.children.length; i++) {
        this.children[i].iterate_postorder(fun);
    }
    fun(this);
};
