...
function divrenderer(element_id, config){
    ...
    this.leaf_style = {
        ...
        height: ( this.config.box_height
                  - (2 * this.config.leaf_padding)
                  - (2 * this.config.leaf_border) ) + "px",
        "border-radius": "5px"
    };
...
divrenderer.prototype.display = function () {
    ...
    // We don't want to do thatt while we're creating
...
tree.prototype.layout = function() {
    var leaf_counter = 0;
    return [].concat.apply([], this.traverse(
        function(node, down_data) {
            return down_data + node.parent_distance;
        },
        0,
        function(node, child_results, down_data) {
            ...
            var descendant_positions;
            for (var i = 0; i < child_results.length; i++) {
                // child_results will be an array of
                // arrays of position objects, where
                // the position object at index 0
                // is the position of the immediate child
                // (see how result is constructed below)
                immediate_child_y_sum += child_results[i][0].y;
            }
            
            // flatten child result arrays
            var result = [].concat.apply([], child_results);
            result.unshift({
                id: node.id,
                x: down_data,
                y: ( node.is_leaf()
