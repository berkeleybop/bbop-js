...
divrenderer.prototype.display = function () {
	...
	if( is_leaf ) this.leaves.push(phynode);
    ...
    this.tree_height = (this.leaves.length * this.row_height);
...
divrenderer.prototype.width_changed = function() {
    ...
    var potential_widths = [];
    for( var li = 0; li < this.leaves.length; li++ ){
        potential_widths.push(
            (parentWidth - this.leaves[li].node_elem.offsetWidth)
                / (this.leaves[li].px / 100) // this.leaves[li].px is in
                                             // percentage units
        );
    }
